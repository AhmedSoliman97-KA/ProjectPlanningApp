...
# Dropbox Folder Path
DROPBOX_FOLDER_PATH = "/Project_Data"  # Change this to your desired folder in Dropbox

# Dropbox Functions
def save_to_dropbox(file_path, file_name, access_token, dropbox_folder_path):
    """Uploads a file to Dropbox."""
    try:
        dbx = dropbox.Dropbox(access_token)
        with open(file_path, "rb") as f:
            dbx.files_upload(f.read(), f"{dropbox_folder_path}/{file_name}", mode=dropbox.files.WriteMode("overwrite"))
        print(f"File {file_name} uploaded to Dropbox at {dropbox_folder_path}.")
    except dropbox.exceptions.ApiError as e:
        print(f"Dropbox API error: {e}")
    except Exception as e:
        print(f"An error occurred: {e}")

def download_from_dropbox(file_name, access_token, dropbox_folder_path):
    """Downloads a file from Dropbox."""
    try:
        dbx = dropbox.Dropbox(access_token)
        metadata, res = dbx.files_download(f"{dropbox_folder_path}/{file_name}")
...
def save_project_data(dataframe, file_path):
    ...
    # Upload to Dropbox
    save_to_dropbox(file_path, os.path.basename(file_path), ACCESS_TOKEN, DROPBOX_FOLDER_PATH)
...
def main():
    ...
    if st.button("Download Latest File"):
        try:
            download_from_dropbox("Project_Data.xlsx", ACCESS_TOKEN, DROPBOX_FOLDER_PATH)
